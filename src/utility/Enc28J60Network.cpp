...
// set CS to 0 = active
#define CSACTIVE HAL_GPIO_WritePin(ETH_CS_GPIO_Port, ETH_CS_Pin, GPIO_PIN_RESET)
// set CS to 1 = passive
#define CSPASSIVE HAL_GPIO_WritePin(ETH_CS_GPIO_Port, ETH_CS_Pin, GPIO_PIN_SET)
...
uint8_t Enc28J60Network::readOp(uint8_t op, uint8_t address) {
    CSACTIVE;
    // issue read command
    uint8_t command = op | (address & ADDR_MASK);
    uint8_t result = 0;
    HAL_SPI_Transmit(&hspi1, &command, 1, 100);
    if (address & 0x80) {
...
uint16_t Enc28J60Network::chksum(uint16_t sum, memhandle handle, memaddress pos, uint16_t len) {
    ...
    for (i = 0; i < len; i += 2) {
        ...
        HAL_SPI_Receive(&hspi1, &part, 1, 100);
        t = part << 8;
    ...
    if (i == len) {
        uint8_t last;
        HAL_SPI_Receive(&hspi1, (uint8_t*) &last, 1, 100);
        t = (last << 8) + 0;
